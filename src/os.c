...
static void * cpu_routine(void * args) {
...
#ifdef SUMMARY
        struct log_pointer * log=NULL;
        struct log_pointer * tail=NULL;
#endif	
	...
	while (1) {
		...
		if (proc == NULL && done_load) {
			/* No process to run, exit */
			printf("\tCPU %d stopped at %d\n", id, current_time());
#ifdef SUMMARY
                        printf("Grantt chart of CPU %d:\n\t|",id);
                        struct log_pointer* head=log;
                        for (int i=0;i<=current_time();i++){
                           if (log==NULL) continue;
                           if (i!=log->time) printf(" (%d, ) |",i);
                           else {
                              printf(" (%d,%d) |",i,log->pid);
                              log=log->pnext;
                            }
                         }
                         printf("\n");
                         while (head!=NULL){
                              struct log_pointer* temp=head;
                              head=head->pnext;
                              free(temp);
                         }
#endif
		...
		run(proc);
#ifdef SUMMARY
                struct log_pointer *newLog= malloc (sizeof(struct log_pointer));
                newLog->pid=proc->pid;
                newLog->time=current_time();
                newLog->pnext=NULL;
...
int main(int argc, char * argv[]) {
	...
	struct timer_id_t * sche_timer = attach_event();
